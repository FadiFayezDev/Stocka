...
namespace Application.Bases
{
    ...
    public abstract class BaseHandler<Command, Query> : ResponseHandler where Command : class where Query : class
    {
        ...
        public BaseHandler(Command command, Query query, IUnitOfWork work, IMapper? mapper = null)
        {
            _command = command;
            _query = query;
            _work = work;
            ...
            where TDto : class
        {
            try
            {
                ...
                if (!success)
                    throw new BusinessException("Failed to create resource");
            ...
            where TEntity : class
        {
            try
            {
                ...
                var success = await deleteAction(entity);
                if (!success)
                    throw new BusinessException("Failed to delete resource");
