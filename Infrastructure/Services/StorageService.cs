...
namespace Infrastructure.Services
{
    public class StorageService : IStorageService
    {
        ...
        {
            // 1. تحويل الـ URL لـ Uri object عشان نقدر نطلع المسار منه بسهولة
            var uri = new Uri(fileUrl);

            // 2. الحصول على المسار بعد اسم الدومين (سيشمل اسم الكونتينر واسم الملف)
            // النتيجة هنا هتكون: /product-images/edd37651...%5C...png
            string pathInContainer = uri.AbsolutePath.TrimStart('/');

            // 3. إزالة اسم الكونتينر من البداية للحصول على اسم الـ Blob فقط
            // بنفترض إن _conteinerName قيمته "product-images"
            string blobName = pathInContainer.Substring(_conteinerName.Length).TrimStart('/');

            // 4. فك الترميز (Decoding) عشان %5C ترجع لأصلها أو تتعالج
            // وبما أن الكود بتاعك بيستخدم Replace('%', '/') فده معناه إنك عايز تحول أي فاصل لـ Slash
            blobName = Uri.UnescapeDataString(blobName).Replace('\\', '/');
